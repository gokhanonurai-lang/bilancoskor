...
export default function SampleReportModal({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => {
    fetch('/api/sample-report')
      .then(r => r.json())
      .then(data => {
        if (data.error) { setError('Rapor yüklenemedi.'); setLoading(false); return }
        setMeta(data)
      ...
      <div className="bg-white rounded-3xl w-full max-w-3xl max-h-[92vh] flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 flex-shrink-0">
          <div>
            ...
            <div className="text-xs text-gray-400">
              {meta?.firma_adi || 'Örnek Firma'} · {meta?.sektor ? meta.sektor.charAt(0).toUpperCase() + meta.sektor.slice(1) : 'Ticaret'} Sektörü
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
          {loading ? (
            ...
            <div className="flex items-center justify-center py-20">
              <p className="text-sm text-gray-500">{error || 'Rapor yüklenemedi.'}</p>
            </div>
          ) : (
            <>
              {/* 1. YÖNETİCİ ÖZETİ */}
                    ...
                    {[
                      ...
                      <div key={x.k} className="bg-white rounded-xl p-3">
                        ...
                        <div className="mt-1.5 bg-gray-100 rounded-full h-1.5 overflow-hidden">
                          <div className="h-full bg-brand-400 rounded-full" style={{ width: `${Math.min(100, (x.p / x.m) * 100)}%` }} />
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {rapor.kredi_turu && (
                <section>
                  <ST num="5" title="Finansman Araçlarına İlişkin Genel Bilgilendirme" />
              ...
              {rapor.rasyolar?.length > 0 && (
                <section>
                  <ST num="7" title={`Rasyo Analizleri — ${rapor.rasyolar.length} Rasyo, Kategori Bazlı`} />
                  <div className="space-y-3">
              ...
              {rapor.alt_hesap_analizi?.length > 0 && (
                <section>
                  <ST num="8" title="Alt Hesap Analizi" />
                  <div className="space-y-4">
                    {rapor.alt_hesap_analizi.map((h: any, i: number) => (
                      <div key={i} className="border border-gray-100 rounded-2xl p-4">
                        <div className="font-semibold text-gray-900 mb-2 text-sm">{h.ana_hesap_kodu} — {h.ana_hesap_adi}</div>
                        {h.uyari_notu && (
                          <div className="text-xs text-amber-600 bg-amber-50 rounded-lg px-3 py-2 mb-2">⚠ {h.uyari_notu}</div>
                        )}
                        <div className="text-sm text-gray-600 leading-relaxed whitespace-pre-line">{h.analiz_metni}</div>
                      </div>
                    ))}
                  </div>
                </section>
              ...
              {rapor.senaryolar !== undefined && (
                <section>
                  <ST num="9" title="Senaryo Motoru" />
                  {rapor.senaryolar.length === 0 ? (
                    <p className="text-sm text-gray-400 text-center py-6">Finansal göstergeler sektör normlarında; belirgin iyileştirme senaryosu tespit edilmedi.</p>
                  ) : (
                    <>
              ...
              {/* 10. BANKA GÖRÜŞME SORULARI */}
              {rapor.banka_sorulari?.length > 0 && (
                <section>
                  <ST num="10" title="Banka Görüşme Soruları" />
                  <div className="space-y-4">
              ...
              {/* 11. BANKA BAŞVURU HAZIRLIĞI */}
              {rapor.banka_hazirlik && (
                <section>
                  <ST num="11" title="Banka Başvuru Hazırlığı" />
                    ...
                    <div>
                      ...
                      <ul className="space-y-2">
                        {(rapor.banka_hazirlik.dikkat_edilecekler || []).map((d: string, i: number) => (
                          <li key={i} className="flex items-start gap-2 text-sm text-gray-700">
                            <span className="text-amber-500 font-bold flex-shrink-0">!</span>{d}
                          </li>
                        ))}
                      </ul>
                    </div>
              ...
              {/* 12. AKSİYON ZAMAN ÇİZELGESİ */}
              {rapor.zaman_cizelgesi?.length > 0 && (
                <section>
                  <ST num="12" title="Aksiyon Zaman Çizelgesi" />
                  <div className="space-y-4">
              ...
              {/* 13. MODEL SKORU VE POTANSİYEL */}
              {rapor.potansiyel_raporu && (
                <section>
                  <ST num="13" title="Model Skorunu Etkileyen Faktörler" />
                  ...
                  <div className="text-sm text-gray-600 leading-relaxed whitespace-pre-line mb-4">{rapor.potansiyel_raporu}</div>
                  {rapor.oncelik_tablosu?.length > 0 && (
        ...
        <div className="border-t border-gray-100 px-6 py-4 flex items-center justify-between flex-shrink-0 bg-gray-50">
          ...
          <button onClick={onClose} className="text-xs text-gray-500 hover:text-gray-800 transition">Kapat</button>
        </div>
      </div>
    </div>
  )
